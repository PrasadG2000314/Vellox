...
function initShaderProgram(gl: WebGLRenderingContext, vs: string, fs: string) {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error("[v0] Shader program link error:", gl.getProgramInfoLog(program))
    return null
  }
  return program
}
...
export default function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    console.log("[ShaderBackground] Theme changed to:", resolvedTheme)
  }, [resolvedTheme])
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      gl.uniform4fv(programInfo.uniformLocations.lineColor, lineCol)
  ...
  return <canvas ref={canvasRef} className={className ?? "fixed left-0 top-0 -z-10 h-full w-full"} aria-hidden="true" />
}
